...
Unit Tests
===========
The units library has a series of unit tests that are executed as part of the CI builds and when developing.  They are built using Google Test module.  The module is downloaded if the tests are built.  It uses Release 1.10 from the Google Test GitHub repository
...
5.  `test_commodities`  Runs tests using the commodity-related functions and operations on precise_units
...
7.  `test_conversions2` Runs through a series of test units and conversions from one of the converter websites; there are a number of files that get used that contain known conversions
...
15. `test_udunits` a series of tests and test files coming from  `UDUNITS-2 <https://github.com/Unidata/UDUNITS-2>`_  Not all units convert, some never will since they are ambiguous but we will probably allow a few more over time
